...
\begin{remark}[Imputation vs.\ Real-Time Availability]
Median imputation yields a rectangular numerical matrix required for linear and tree-based estimators, but it does not generate observed historical data. Furthermore, while the DMA target update strictly respects calendar release dates ($t_0 + h \le t$), features are indexed by nominal country-year $t$. In operational multilateral surveillance, official annual national accounts and V-Dem indicators are published with reporting lags (3 to 12 months post-period) and undergo subsequent statistical revisions. Operational real-time deployment would necessitate vintage-aware databases and nowcasting pipelines.
\end{remark}
...
We emphasize the foundational econometric distinction: rejection of the Dumitrescu-Hurlin Granger non-causality null establishes \textbf{predictive precedence} (temporal informational content in past lags), not structural, simultaneous, or counterfactual causality.
\end{remark}
...
\begin{enumerate}[leftmargin=*, label=\arabic*.]
    \item \textbf{Cross-sectional dependence and common factor filtering}: While our vector-resampling bootstrap and CS-DH tests successfully control for severe CSD ($\hat{CD} > 85$), common global factor filtering attenuates the predictive precedence of biophysical temperature anomalies ($p_{\text{Holm}} = 0.0503$). This indicates that cross-domain indicators with high global synchronization primarily convey common macroeconomic cycles rather than sovereign-specific structural signals.
    \item \textbf{Feature vintages and publication delays}: While target feedback in the DMA router is calendar-gated ($t_0 + h \le t$), features are indexed by nominal country-year $t$. In operational multilateral surveillance, official annual national accounts and V-Dem indicators are published with reporting delays (3 to 12 months post-period) and undergo subsequent revisions. True real-time operational deployment requires vintage databases and nowcasting pipelines.
...
\begin{enumerate}[leftmargin=*, label=\arabic*.]
    ...
    \item \textbf{Copernicus ERA5 \& Global Carbon Budget}: ERA5 surface temperature anomalies (degrees C above the 1960--1990 baseline) and Global Carbon Budget CO$_2$ emissions per capita. Each is augmented with lags and differences, yielding 10 features.
\end{enumerate}
